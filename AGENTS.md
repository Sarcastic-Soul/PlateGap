...
`docs/repo-reference.md` has the file-by-file map and the list of generator-owned files (never
hand-edit those). Quick version: `solver/` (simplex engine + LP/plan/audit/menu-text/scan/explain
modules), `lambda/handler.py` (single entry point, action-dispatched), `web/` (Preact+htm, no
build step), `data/` (catalog + presets + field-study data, mostly generated), `scripts/`,
